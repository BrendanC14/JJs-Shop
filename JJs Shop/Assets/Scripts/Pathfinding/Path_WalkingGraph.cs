...
public class Path_WalkingGraph {
    ...
    bool isClippingCorner(Tile currTile, Tile neighbour)
    {
        ...
        {
            //We are diagonal
            if (World.Current.GetTileAt(currTile.X - dX, currTile.Y).PersonMovementCost == 0)
            {
                //East or West is unwalkable, would be clipped
                return true;
            }
            if (World.Current.GetTileAt(currTile.X, currTile.Y - dY).PersonMovementCost == 0)
            {
